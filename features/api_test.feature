...
Feature: Testing on a mocked-up API
    ...
    Scenario: Get a non-existing item and verify response
        When I launch the petition to obtain an item with ID 9999
    ...
    Scenario: Request items without a valid token and verify unauthorized access
        When I launch the petition to obtain all the items without a token
        Then I verify that the response returns an error for missing or invalid token

...
class TextBoxPage(BasePage):
    ...
    async def fill_and_verify_form(
        self, name, email, current_address, permanent_address
    ):
    ...
    async def verify_submitted_data(
        ...
        assert (
            expected_name[:25] in f"Name:{name_text}"
        ), f"❌ Nombre incorrecto: esperado '{expected_name}', obtenido '{name_text}'"
        assert (
            expected_email in f"Email:{email_text}"
        ), f"❌ Email incorrecto: esperado '{expected_email}', obtenido '{email_text}'"
        assert (
            expected_address in f"Current Address :{address_text}"
        ), f"❌ Dirección incorrecta: esperado '{expected_address}', obtenido '{address_text}'"
        assert (
            expected_permanent_address
            in f"Permananet Address :{permanent_address_text}"
        ), f"❌ Dirección permanente incorrecta: esperado '{expected_permanent_address}', obtenido '{permanent_address_text}'"
